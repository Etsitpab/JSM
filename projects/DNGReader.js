...
(function () {
    'use strict';
    this.DNGReader = {};
    this.DNGReader.LightSources = {
        "0":   {"name": "Unknown", "temperature": 0},
        "1":   {"name": "Daylight", "temperature": 5500},
        "2":   {"name": "Fluorescent", "temperature": (3800.0 + 4500.0) * 0.5},
        "3":   {"name": "Tungsten (incandescent light)", "temperature": 2850},
        "4":   {"name": "Flash", "temperature": 5500},
        "9":   {"name": "Fine weather", "temperature": 5500},
        "10":  {"name": "Cloudy weather", "temperature": 6500},
        "11":  {"name": "Shade", "temperature": 7500},
        "12":  {"name": "Daylight fluorescent (D 5700 - 7100K)", "temperature": (5700.0 + 7100.0) * 0.5},
        "13":  {"name": "Day white fluorescent (N 4600 - 5400K)", "temperature": (4600.0 + 5500.0) * 0.5},
        "14":  {"name": "Cool white fluorescent (W 3900 - 4500K)", "temperature": (3800.0 + 4500.0) * 0.5},
        "15":  {"name": "White fluorescent (WW 3200 - 3700K)", "temperature": (3250.0 + 3800.0) * 0.5},
        "16":  {"name": "Warm white fluorescent (L 2600 - 3250K)", "temperature": (2600.0 + 3250.0) * 0.5},
        "17":  {"name": "Standard light A", "temperature": 2850},
        "18":  {"name": "Standard light B", "temperature": 5500},
        "19":  {"name": "Standard light C", "temperature": 6500},
        "20":  {"name": "D55", "temperature": 5500},
        "21":  {"name": "D65", "temperature": 6500},
        "22":  {"name": "D75", "temperature": 7500},
        "23":  {"name": "D50", "temperature": 5000},
        "24":  {"name": "ISO studio tungsten", "temperature": 3200},
        "255": {"name": "Other light source", "temperature": 0},
    };
    ...
    this.DNGReader.isDNG = function (file) {
        var view = new DataView(file),
            byteOrder = tab2char([view.getUint8(0), view.getUint8(1)]) === "II" ? true : false,
            checkNumber = view.getUint16(2, byteOrder);
        return checkNumber === 42;
    };

    this.DNGReader.readDNG = function (RAW) {
        'use strict';
        ...
        var readIFD = function (offset, infos, entries) {
            ...
            while (offset !== 0) {
                ...
                //if (infos.PhotometricInterpretation && infos.PhotometricInterpretation.value === 32803) {

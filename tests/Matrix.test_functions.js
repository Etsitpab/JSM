...
(function (Matrix, Matrix_prototype) {
	...
	s = Matrix.ones(SQN, SQN, SQN, SQN * 3).cumsum(dim)["-"](1); 
        Tools.tic();
	...
	out = Matrix.ifft(fft);
	time = Tools.toc()
	psnr = s["-"](out)[".^"](2).abs().mean().getDataScalar().toFixed(2);
	...
	out = Matrix.ifft2(fft);
        time = Tools.toc()
	psnr = s["-"](out)[".^"](2).mean().getDataScalar().toFixed(2);
